...
//        DB::query("DELETE FROM community_chat WHERE time < SUBTIME(NOW(),'0:5:0')");
    ...
    public static function getChats($lastID) {
        ...
        $result = DB::query('SELECT cc.`id`, cc.`user_id`,concat(u.firstname," ",u.lastname) as name, cc.`text`, cc.`time` FROM `community_chat` as cc JOIN user_personal_info as u ON cc.user_id=u.id WHERE  cc.id > ' . $lastID . ' ORDER BY cc.id ASC');
        include_once './GossoutUser.php';
        $user = new GossoutUser(0);
        $chats = array();
